...
assert.deepEqual(
  offenders,
  [],
  `dist must not import node: builtins, found: ${offenders}`,
);
...
assert.match(
  JSON.stringify(satim),
  /\[REDACTED\]/,
  'credentials must be redacted in JSON',
);
...
assert.deepEqual(await handler.inspect({ garbage: true }), {
  verified: false,
  reason: 'invalid_source',
});

console.log(
  'smoke: dist loads in plain Node ESM, no node: imports, public API behaves',
);
