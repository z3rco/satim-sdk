...
describe('CircuitBreaker', () => {
  ...
  test('circuit opens after threshold consecutive transient failures', async () => {
    ...
    try {
      ...
      const err = await client
        .handleApiRequest('/register.do', { userName: 'u', password: 'p' })
        .catch((e) => e);
      expect(err).toBeInstanceOf(SatimUnexpectedResponseError);
      expect((err as SatimUnexpectedResponseError).errorCategory).toBe(
        'circuit_open',
      );
  ...
  test('circuit resets to CLOSED after a successful request', async () => {
    ...
    try {
      ...
      shouldSucceed = true;
      await expect(
        client.handleApiRequest('/register.do', {}),
...
describe('Configurable timeout', () => {
  test('HttpClientService accepts custom timeoutMs', () => {
    expect(
      () => new HttpClientService(false, { timeoutMs: 10_000 }),
    ).not.toThrow();
    expect(
      () => new HttpClientService(false, { timeoutMs: 1_000 }),
    ).not.toThrow();
    expect(
      () => new HttpClientService(false, { timeoutMs: 300_000 }),
    ).not.toThrow();
  });

  test('HttpClientService rejects timeoutMs < 1000', () => {
    expect(() => new HttpClientService(false, { timeoutMs: 999 })).toThrow(
      SatimInvalidArgumentError,
    );
    expect(() => new HttpClientService(false, { timeoutMs: 0 })).toThrow(
      SatimInvalidArgumentError,
    );
  });

  test('HttpClientService rejects timeoutMs > 300000', () => {
    expect(() => new HttpClientService(false, { timeoutMs: 300_001 })).toThrow(
      SatimInvalidArgumentError,
    );
  });
  ...
  test('HttpClientService rejects non-numeric timeoutMs', () => {
    expect(
      () => new HttpClientService(false, { timeoutMs: '30000' as any }),
    ).toThrow(SatimInvalidArgumentError);
    expect(() => new HttpClientService(false, { timeoutMs: NaN })).toThrow(
      SatimInvalidArgumentError,
    );
    expect(() => new HttpClientService(false, { timeoutMs: Infinity })).toThrow(
      SatimInvalidArgumentError,
    );
  });
...
describe('WebhookHandler, multi-instance warning', () => {
  ...
  const makeSatim = () =>
    new Satim({ username: 'u', password: 'p', terminalId: 't' });
...
describe('Runtime typeof guards, amount()', () => {
  ...
  test('rejects object input', () => {
    expect(() => satim.amount({} as any)).toThrow(SatimInvalidArgumentError);
    expect(() => satim.amount({ valueOf: () => 100 } as any)).toThrow(
      SatimInvalidArgumentError,
    );
  });
...
describe('Runtime typeof guards, confirm() expectedAmount', () => {
  ...
  test('rejects array, object, null, string, boolean', async () => {
    await expect(satim.confirm('order-123', [100] as any)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
    await expect(satim.confirm('order-123', {} as any)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
    await expect(satim.confirm('order-123', null as any)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
    await expect(satim.confirm('order-123', '100' as any)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
    await expect(satim.confirm('order-123', true as any)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
  });
});
...
describe('Runtime typeof guards, refund() amount', () => {
  ...
  test('rejects array, object, null, string, boolean', async () => {
    await expect(satim.refund('order-123', [100] as any)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
    await expect(satim.refund('order-123', null as any)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
    await expect(satim.refund('order-123', '100' as any)).rejects.toThrow(
      SatimInvalidArgumentError,
...
describe('Runtime typeof guards, orderId', () => {
  ...
  test('confirm() rejects non-string orderId', async () => {
    await expect(satim.confirm(123 as any, 100)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
    await expect(satim.confirm(null as any, 100)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
    await expect(satim.confirm(['order-1'] as any, 100)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
  });

  test('refund() rejects non-string orderId', async () => {
    await expect(satim.refund(123 as any, 100)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
  });

  test('status() rejects non-string orderId', async () => {
    await expect(satim.status(123 as any)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
  });

  test('reverseOrder() rejects non-string orderId', async () => {
    await expect(satim.reverseOrder(123 as any)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
  });
});
