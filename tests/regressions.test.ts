...
describe('circuit breaker counts every transport failure mode', () => {
  test('opens on connection-level failures (fetch rejects with TypeError)', async () => {
    ...
    for (let i = 0; i < 2; i++) {
      await expect(
        client.handleApiRequest('/getOrderStatus.do', { orderId: `n${i}` }),
      ).rejects.toThrow();
    }
    await expect(
      client.handleApiRequest('/getOrderStatus.do', { orderId: 'n3' }),
  ...
  test('opens on malformed payloads served with HTTP 200', async () => {
    ...
    for (let i = 0; i < 2; i++) {
      await expect(
        client.handleApiRequest('/getOrderStatus.do', { orderId: `p${i}` }),
      ).rejects.toThrow();
    }
    await expect(
      client.handleApiRequest('/getOrderStatus.do', { orderId: 'p3' }),
    ).rejects.toThrow(/Circuit breaker is open/);
  });
  ...
  test('does not open on 4xx, a client-side fault, not gateway degradation', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      await expect(
        client.handleApiRequest('/getOrderStatus.do', { orderId: `c${i}` }),
  ...
  test('a failed probe still reopens the breaker rather than admitting a herd', async () => {
    ...
    await expect(
      client.handleApiRequest('/getOrderStatus.do', { orderId: 'a' }),
    ).rejects.toThrow();
    await new Promise((r) => setTimeout(r, 40));
    // Probe is admitted and fails, so the breaker must be closed again to traffic.
    await expect(
      client.handleApiRequest('/getOrderStatus.do', { orderId: 'b' }),
    ).rejects.toThrow(/Network/);
    await expect(
      client.handleApiRequest('/getOrderStatus.do', { orderId: 'c' }),
    ).rejects.toThrow(/Circuit breaker is open/);
  });
  ...
  test('the TLS guard neither counts as a failure nor consumes a probe', async () => {
    ...
    try {
      const client = clientWith(
        async () => jsonResponse({ OrderStatus: '2' }),
        { circuitBreaker: { failureThreshold: 2, resetTimeoutMs: 10_000 } },
      );
      for (let i = 0; i < 5; i++) {
        await expect(
          client.handleApiRequest('/getOrderStatus.do', { orderId: `t${i}` }),
        ).rejects.toThrow(SatimError);
      }
      // Fixing the environment must not leave a circuit open behind it.
      process.env.NODE_TLS_REJECT_UNAUTHORIZED = '1';
      await expect(
        client.handleApiRequest('/getOrderStatus.do', { orderId: 'ok' }),
      ).resolves.toBeTruthy();
    } finally {
...
describe('timeout classification is independent of the fetch implementation', () => {
  test('a custom fetch rejecting with a plain AbortError is treated as a timeout', async () => {
    ...
    const error = await client
      .handleApiRequest('/getOrderStatus.do', { orderId: 'x' })
      .then(
        () => null,
        (e: SatimUnexpectedResponseError) => e,
      );
 ...
 */
function stubbedSatim(
  handleApiRequest: (endpoint: string, data: any, opts?: any) => Promise<any>,
) {
...
describe('webhook marks only definitively terminal orders', () => {
  ...
  test('the replay path still verifies the amount', async () => {
    const satim = stubbedSatim(() =>
      Promise.resolve({ OrderStatus: '2', Amount: 999999 }),
    );
    const handler = satim.createWebhookHandler({
      allowUnverifiedCallbacks: true,
      onResolveAmount: () => 100,
    ...
    await expect(handler.verify({ orderId: 'ORDER6' })).rejects.toThrow(
      /mismatch/,
    );
  });
});
...
describe('webhook rejections are distinguishable', () => {
  function handlerWithLimit(max: number) {
    const satim = stubbedSatim(() =>
      Promise.resolve({ OrderStatus: '2', Amount: 10000 }),
    );
  ...
  test('inspect() separates junk, rate limiting, and unknown orders', async () => {
    ...
    expect(await handler.inspect({ nope: true })).toEqual({
      verified: false,
    ...
    expect(await handler.inspect({ orderId: 'OTHER' })).toEqual({
      verified: false,
    ...
    expect(await handler.inspect({ orderId: 'KNOWN' })).toEqual({
      verified: false,
      reason: 'rate_limited',
    });
  });
  ...
  test('malformed sources do not consume rate-limit budget', async () => {
    ...
    for (let i = 0; i < 10; i++)
      await handler.inspect('!!! not an order id !!!');
...
describe('gateway numeric fields are accepted on both schemas', () => {
  test('a registration carrying a numeric errorCode is not rejected', async () => {
    ...
    const satim = stubbedSatim(() =>
      Promise.resolve({
        orderId: 'ord-1',
        formUrl: 'https://test.satim.dz/pay/x',
        errorCode: 0,
      }),
    );
    const res = await satim
      .amount(5000)
      .returnUrl('https://shop.dz/r')
      .register();
  ...
  test('string errorCode and absent errorCode still work', async () => {
    for (const extra of [{ errorCode: '0' }, {}]) {
      const satim = stubbedSatim(() =>
        Promise.resolve({
          orderId: 'ord-2',
          formUrl: 'https://test.satim.dz/pay/y',
          ...extra,
        }),
      );
      const res = await satim
  ...
  test('a non-scalar errorCode is still rejected', async () => {
    const satim = stubbedSatim(() =>
      Promise.resolve({
        orderId: 'ord-3',
        formUrl: 'https://test.satim.dz/pay/z',
        errorCode: { nested: true },
      }),
    );
    await expect(
      satim.amount(5000).returnUrl('https://shop.dz/r').register(),
...
describe('baseUrl override', () => {
  test('replaces the host the endpoint is appended to', async () => {
    ...
    const client = new HttpClientService(false, {
      ...
      fetch: (async (url: any) => {
        seen = String(url);
        return new Response(JSON.stringify({ OrderStatus: 2 }), {
          status: 200,
        });
      }) as any,
    });
  ...
  test('omitting it keeps the built-in hosts', async () => {
    ...
    await new HttpClientService(false, { fetch: fetchImpl }).handleApiRequest(
      '/getOrderStatus.do',
      {},
    );
    await new HttpClientService(true, { fetch: fetchImpl }).handleApiRequest(
      '/getOrderStatus.do',
      {},
    );
...
describe('allowPrivateUrls', () => {
  ...
  test('loopback callback URLs are rejected by default', () => {
    for (const url of [
      ...
      expect(() => satim().dynamicCallbackUrl(url)).toThrow(
        /private\/reserved/,
      );
    }
  });
  ...
  test('the rejection tells the developer how to proceed', () => {
    expect(() => satim().returnUrl('http://localhost:3000/cb')).toThrow(
  ...
  test('opting in allows them on all three URL setters', () => {
    ...
    expect(() =>
      dev.dynamicCallbackUrl('http://127.0.0.1:3000/cb'),
    ).not.toThrow();
  });
  ...
  test('it is per-instance and does not leak to other clients', () => {
    ...
    expect(() => satim().returnUrl('http://localhost:3000/return')).toThrow(
      /private\/reserved/,
    );
  });
  ...
  test('turning it back off restores the guard', () => {
    const dev = satim().allowPrivateUrls(true);
    expect(() =>
      dev.allowPrivateUrls(false).returnUrl('http://localhost:3000/cb'),
    ).toThrow();
  });
...
describe('bad credentials raise the same error whichever endpoint answers', () => {
  test('HTTP 401 with a bare string body maps to SatimInvalidCredentialsError', async () => {
    ...
    const client = new HttpClientService(false, {
      baseUrl: 'https://gw.satim.dz/payment/rest',
      fetch: (async () =>
        new Response('"Access denied"', { status: 401 })) as any,
    });
    await expect(
      client.handleApiRequest('/public/acknowledgeTransaction.do', {}),
  ...
  test('403 maps the same way, other 4xx do not', async () => {
    ...
    await expect(
      make(403).handleApiRequest('/refund.do', {}),
    ).rejects.toBeInstanceOf(SatimInvalidCredentialsError);
    await expect(make(404).handleApiRequest('/refund.do', {})).rejects.toThrow(
      /HTTP Error: 404/,
    );
  });
  ...
  test('a credential failure does not open the circuit breaker', async () => {
    ...
    for (let i = 0; i < 4; i++) {
      await expect(
        client.handleApiRequest('/refund.do', {}),
      ).rejects.toBeInstanceOf(SatimInvalidCredentialsError);
    }
  });
});
...
describe('deposit / decline / statusExtended', () => {
  ...
  test('deposit validates its inputs', async () => {
    ...
    await expect(satim.deposit('bad id!')).rejects.toThrow(
      SatimInvalidArgumentError,
    );
    await expect(satim.deposit('order-1', -5)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
    await expect(satim.deposit('order-1', 1.234)).rejects.toThrow(
      SatimInvalidArgumentError,
    );
  });
...
describe("the gateway's real callback format", () => {
  ...
  test('mdOrder is extracted, BPC callbacks carry no orderId at all', () => {
    ...
    expect(
      extractOrderId({ mdOrder: ID, operation: 'deposited', status: '1' }),
    ).toBe(ID);
  ...
  test("checksum verification matches node:crypto's HMAC", async () => {
    ...
    const checksum = createHmac('sha256', secret)
      .update(buildSignedString(params))
      .digest('hex')
      .toUpperCase();

    expect(verifyCallbackChecksum({ ...params, checksum }, secret)).toBe(true);
    expect(
      verifyCallbackChecksum({ ...params, amount: '1', checksum }, secret),
    ).toBe(false);
    expect(
      verifyCallbackChecksum({ ...params, checksum }, 'wrong-secret'),
    ).toBe(false);
      ...
      handleApiRequest: (endpoint: string) => {
        ...
        if (code === '5')
          return Promise.reject(
            new SatimInvalidCredentialsError('Access denied'),
          );
        if (code === '6')
          return Promise.reject(
            new SatimInvalidArgumentError('Invalid order ID'),
          );
        if (code === '404') {
          return Promise.reject(
            new SatimUnexpectedResponseError(
              'HTTP Error: 404',
              'http',
              undefined,
              { httpStatus: 404 },
        ...
        return Promise.reject(
          new SatimUnexpectedResponseError('boom', 'gateway'),
        );
      },
    };
    return satim;
...
describe('a permission-gated refusal explains itself', () => {
  const denied = () =>
    new HttpClientService(false, {
      ...
      fetch: (async () =>
        new Response(
          JSON.stringify({ ErrorCode: 5, ErrorMessage: 'Access denied' }),
          { status: 200 },
        )) as any,
    });
  ...
  test('refund mentions terminal entitlement, not just the password', async () => {
    await expect(
      denied().handleApiRequest('/refund.do', {}, { retryable: false }),
...
describe('duplicate query keys are rejected (parser differential)', () => {
  ...
  test('any repeated key at all is rejected', () => {
    expect(
      extractParams(`https://shop.dz/cb?mdOrder=${ID}&amount=1&amount=2`),
    ).toBeNull();
  });
  test('a single-valued callback still works', () => {
    expect(extractOrderId(`https://shop.dz/cb?mdOrder=${ID}`)).toBe(ID);
    expect(extractParams(`https://shop.dz/cb?mdOrder=${ID}&amount=1`)).toEqual({
      mdOrder: ID,
      amount: '1',
    });
  });
...
describe('getUrl() enforces the same allowlist as redirectResponse()', () => {
  const make = (formUrl: string) =>
    new RegisterResponse({ orderId: 'o', formUrl } as any);
  test('a trusted satim.dz form URL is returned', () => {
    expect(make('https://test.satim.dz/pay/x').getUrl()).toBe(
      'https://test.satim.dz/pay/x',
    );
  });
  test('an attacker-controlled origin is rejected by getUrl(), not just redirectResponse()', () => {
    ...
    expect(() => make('https://satim.dz.evil.com/pay').getUrl()).toThrow(
      /Untrusted/,
    );
...
describe('response body is capped', () => {
  test('an oversized body is rejected rather than buffered', async () => {
    ...
    await expect(
      client.handleApiRequest('/getOrderStatus.do', {}),
  ...
  test('a normal body passes', async () => {
    const client = new HttpClientService(false, {
      baseUrl: 'https://gw.satim.dz/payment/rest',
      fetch: (async () =>
        new Response(JSON.stringify({ OrderStatus: '2' }), {
          status: 200,
        })) as any,
    });
    await expect(
...
describe('confirm() cross-checks the settled currency', () => {
  test('a currency mismatch on a successful payment throws', async () => {
    ...
    (s as any).httpClientService = {
      handleApiRequest: () =>
        Promise.resolve({ OrderStatus: '2', Amount: 10000, currency: '840' }),
    };
    await expect(
  ...
  test('a matching currency is fine', async () => {
    ...
    (s as any).httpClientService = {
      handleApiRequest: () =>
        Promise.resolve({ OrderStatus: '2', Amount: 10000, currency: '012' }),
    };
    await expect(
      s.confirm('11111111-1111-1111-1111-111111111111', 100),
    ).resolves.toBeTruthy();
