...
export class HttpClientService {
  ...
  constructor(
    ...
    if (
      ...
      throw new SatimInvalidArgumentError(
        'timeoutMs must be a number between 1000 and 300000 ms.',
      );
    }
    this.timeoutMs = t;
    this.circuitBreaker =
  ...
  public handleApiRequest<T = unknown>(
    ...
    if (!retryable) {
      return this.sendRequest<T>(endpoint, data, false).then((result) => {
        this.validateApiResponse(result, endpoint);
        return result;
      });
    }
  ...
  private async sendRequest<T>(
    ...
    for (let attempt = 0; attempt <= maxAttempts; attempt++) {
      if (attempt > 0) {
        await new Promise((r) =>
          setTimeout(r, this.getRetryDelay(attempt - 1)),
        );
      }
      ...
      try {
        ...
        if (
          error instanceof SatimError &&
          !(error instanceof SatimUnexpectedResponseError)
        ) {
          throw error;
        }
    ...
    throw (
      lastError ??
      new SatimUnexpectedResponseError('Request failed after retries')
    );
  }
  ...
  private validateApiResponse(response: unknown, endpoint = ''): void {
    ...
    throw new SatimUnexpectedResponseError(
      `Gateway error (code ${code}): ${sanitizeGatewayMessage(message)}`,
      'gateway',
    );
  }
}
