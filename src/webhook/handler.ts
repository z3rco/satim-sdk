...
export interface WebhookHandlerOptions {
  onResolveAmount: (
    orderId: string,
  ) => Promise<number | undefined | null> | number | undefined | null;
...
export class WebhookHandler {
  ...
  private readonly inflightLocks = new Map<
    string,
  ...
  constructor(satim: Satim, options: WebhookHandlerOptions) {
    ...
    this.onCheckDuplicate =
      options.onCheckDuplicate ?? ((id) => this.processedSet.has(id));
    this.onMarkProcessed =
    ...
    if (!Number.isInteger(maxCallbacks) || maxCallbacks < 1) {
      throw new SatimInvalidArgumentError(
        'maxCallbacksPerWindow must be a positive integer.',
      );
    }
    if (!Number.isInteger(windowMs) || windowMs < 1000) {
      throw new SatimInvalidArgumentError(
        'rateLimitWindowMs must be an integer >= 1000.',
      );
    }
  ...
  async inspect(source: unknown): Promise<WebhookOutcome> {
    ...
    if (existing) {
      ...
      return first
        ? {
            verified: true,
            result: { orderId, response: first.response, duplicate: true },
          }
  ...
  private static isTerminal(response: ConfirmResponse): boolean {
    return (
      response.isSuccessful() || response.isRefunded() || response.isReversed()
    );
  }
}
