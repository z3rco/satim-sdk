...
export class Satim extends SatimConfig {
  ...
  constructor(
    credentials: SatimCredentials,
    httpClientService?: HttpClientService | HttpClientOptions,
  ) {
    super();
  ...
  public override setTestMode(isEnabled: boolean): this {
    ...
    if (!clone._hasCustomHttpClient) {
      clone.httpClientService = new HttpClientService(
        clone.testMode,
  ...
  public async checkCapabilities(): Promise<SatimCapabilities> {
    ...
    const [statusExtended, deposit, refund, reverse, decline] =
      await Promise.all([
  ...
  private validateForRegister(): void {
    if (this._returnUrl === undefined) {
      throw new SatimMissingDataError(
        'Return URL missing. Call returnUrl() to set it.',
      );
    }
    if (this._amount === undefined) {
      throw new SatimMissingDataError(
        'Amount missing. Call the amount() method to set it.',
      );
    }
  }
  ...
  private buildData(orderNumber: string): Record<string, unknown> {
    const { [FORCE_TERMINAL_KEY]: _stripped, ...safeUserFields } =
    ...
    const data: Record<string, unknown> = {
      ...
      jsonParams: JSON.stringify({
        ...safeUserFields,
        [FORCE_TERMINAL_KEY]: this.terminalId,
      }),
    };
  ...
  private async registerAt(
    endpoint: '/register.do' | '/registerPreAuth.do',
    ...
    const result =
      await this.httpClientService.handleApiRequest<RegisterOrderResponse>(
        endpoint,
        data,
        { retryable },
      );
  ...
  private async safeRegisterAt(
    merchantRef: string,
    mode: 'register' | 'preauth',
  ): Promise<RegisterResponse> {
    if (!merchantRef || !merchantRef.trim()) {
      throw new SatimInvalidArgumentError(
        `merchantRef is required for ${mode === 'preauth' ? 'safeRegisterPreAuth' : 'safeRegister'}.`,
      );
    }
    this.validateForRegister();
